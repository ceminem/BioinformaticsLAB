...
#ggsave("figs/All downregulated genes GeneRatio .png", width = 12, height = 8)
```

**Answer:** By looking at the graphs,which are sorted by GeneRatio, we can see that **a gene cluster with mobility functions like cell motility or flagellum-dependent cell motility** rank highest in terms of GeneRatio.
...
#ggsave("figs/All downregulated genes Counts .png", width = 12, height = 8, bg="white")
```

**Answer:** By looking at the graphs,which are sorted by Counts, we can see that **the same gene cluster with mobility functions like cell motility or flagellum-dependent cell motility** rank also highest in terms of Counts.
...
```{r}
enrichedBP <- enrichplot::pairwise_termsim(orUpBP)
emapplot(enrichedBP, showCategory = 30, node_label="none")
ggsave("figs/enr_plot_group .png", width = 14, height = 12, bg="white")

#If the parameter group is set TRUE, one can see a grouping
#emapplot(enrichedBP, showCategory = x, node_label="none", group=TRUE)
```
**Answer:** For easier counting, the node labels were removed using nodel_label="none". 

For 10 Categories, we can count 3 clusters.
For 20 Categories, we can count 4 clusters.
For 30 Categories, we can count 5 clusters.
