...
```
\newpage
#Part One: Microbiome (Taxonomic) Analysis
...
=======
>>>>>>> 854e5e0b79cebfc959713c04eaeb06e2e5add133
...
First, we extract two sequences (one from each file):
```{r}
extract_first_fasta <- function(fasta_file) {
  lines <- readLines(fasta_file)
  header_indices <- which(substr(lines, 1, 1) == ">")
...
# Extract first sequence and export
result <- extract_first_fasta("FAP_BLAST.fas")

# Create FASTA format for ColabFold
...
# Save
writeLines(fasta_content, "01_alphafold_input.fasta")
```
